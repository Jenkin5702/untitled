...
def SBX(POP):
    ...
    local_data_offset = np.linspace(0, n, comm_size + 1).astype('int')
    ...
    r1 = np.random.rand(local_n)
    r2 = np.random.rand(local_n, c)
    r3 = np.random.rand(local_n, c)
    A = np.array([np.random.permutation(local_data.shape[0]) for i in range(n)])[:, [0, 1]]
    POP1 = local_data[A[:, 0], :][:, range(c)]
    POP2 = local_data[A[:, 1], :][:, range(c)]
    ...
    NPOP = np.zeros([2 * local_n, c])
    NPOP[2 * np.arange(local_n), :][r2 <= 0.5] = r[0][r2 <= 0.5]
    NPOP[2 * np.arange(local_n) + 1, :][r2 <= 0.5] = r[1][r2 <= 0.5]
    NPOP[2 * np.arange(local_n), :][r2 > 0.5] = POP1[r2 > 0.5]
    NPOP[2 * np.arange(local_n) + 1, :][r2 > 0.5] = POP2[r2 > 0.5]
    NPOP[2 * np.arange(local_n), :][r1 > pc] = 0
    NPOP[2 * np.arange(local_n) + 1, :][r1 > pc] = 0
    combine_data = comm.gather(NPOP, root=0)
    if comm_rank == 0:
        NPOP=np.vstack([i for i in combine_data])
    return NPOP
...
def SBXCD(CA, DA):
    ...
    r2 = np.random.rand(local_n, c)
    r3 = np.random.rand(local_n, c)
    r4 = np.random.rand(local_n, c)
    Ac = np.array([np.random.permutation(CA.shape[0]) for i in range(local_n)])[:, [0, 1]]
    Ad = np.array([np.random.permutation(DA.shape[0]) for i in range(local_n)])[:, [0, 1]]
    [k1, k2] = Ac.T[[0, 1]]
    ...
    DAr = DA[y, :][:, range(c)]
    CAr = CA[k, :][:, range(c)]
    d=np.max([DAr, CAr], 0) - np.min([DAr, CAr], 0)
    d[d==0]=1
    beta = 1 + 2 * np.min([np.min([DAr, CAr], 0), 1 - np.max([DAr, CAr], 0)], 0)/d
    ...
    NPOP[np.arange(local_n) * 2, :][:, np.arange(c)] = 0.5 * (b1 - b2)
    NPOP[np.arange(local_n) * 2 + 1, :][:, np.arange(c)] = 0.5 * (b1 + b2)
    NPOP[NPOP > 1] = 1
    NPOP[NPOP < 0] = 0
    NPOP[np.arange(local_n) * 2, :][:, np.arange(c)][r2 > 0.5] = DAr[r2 > 0.5]
    NPOP[np.arange(local_n) * 2 + 1, :][:, np.arange(c)][r2 > 0.5] = CAr[r2 > 0.5]
    NPOP[np.arange(local_n) * 2, :][:, np.arange(c)][r4 > pc] = 0
    NPOP[np.arange(local_n) * 2 + 1, :][:, np.arange(c)][r4 > pc] = 0
    combine_data = comm.gather(NPOP, root=0)
    if comm_rank == 0:
        NPOP=np.vstack([i for i in combine_data])
    return NPOP
...
def mutation(POP):
    if comm_rank == 0:
        ...
        all_data =POP[randk, :][:, range(c)]
    all_data = comm.bcast(all_data if comm_rank == 0 else None, root=0)
    local_data_offset = np.linspace(0, n, comm_size + 1).astype('int')
    local_n=local_data_offset[comm_rank + 1]-local_data_offset[comm_rank]
    local_data = all_data[local_data_offset[comm_rank]:local_data_offset[comm_rank + 1]]

    delta = np.min(np.array([(local_data - bd) / (bu - bd), (bu - local_data) / (bu - bd)]), 0)
    r2 = np.random.rand(local_n, c)
    indt = np.random.rand(local_n, c)
    yy = abs((2 * r2 + abs(1 - 2 * r2) * ((1 - delta) ** (eta_m + 1))) ** (1 / (eta_m + 1)) - 1) * (bu - bd)
    local_data[local_data <= bd] = (np.random.rand(local_n, c) * (bu - bd) + bd)[local_data <= bd]
    local_data[local_data > bd] = (local_data + yy)[local_data > bd]
    local_data[indt > pm] = POP[randk, :][:, range(c)][indt > pm]
